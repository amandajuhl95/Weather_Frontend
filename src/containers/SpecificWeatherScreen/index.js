...
export default () => {
  ...
  function rain(container, svg_path, opts = {}) {
    const o = {
      density: 1e3,
      min_duration: 3e3,
      max_duration: 5e3,
      easing: "ease-in",
      spawnsvg_uncertainty: 500,
      min_rotation: 200,
      max_rotation: 1e3,
      min_size: 30,
      max_size: 60,
      start_color: "#94c3ffff",
      end_color: "#ffc3fff0",
      full_screen: true
    };
    ...
    if (o.full_screen) {
      w =
        o.max_size +
        Math.max(
          document.documentElement.clientWidth - o.max_size,
          window.innerWidth - o.max_size || 0
        );
      h = Math.max(
    ...
    let css_string = `\n        position: absolute;\n        top: -${
      ...
      o.full_screen
        ? h + o.max_size + "px"
        : "calc(100% + " + o.max_size + "px)"
    };\n        width: 100%;\n        left: 0px;\n        overflow: hidden;\n        z-index: 1;\n    `;
    subcontainer.setAttribute("style", css_string);
    if (!o.full_screen) container.style.position = "relative";
    container.appendChild(subcontainer);
    const create_svg = () => {
      const svg = document.createElementNS("http://www.w3.org/2000/svg", "svg");
      const x = 100 + Math.floor(Math.random() * w - 200);
      const size =
        o.min_size + Math.floor(Math.random() * (o.max_size - o.min_size));
      const duration =
        o.min_duration +
        Math.floor(Math.random() * (o.max_duration - o.min_duration));
      const duration_css = (duration / 1e3).toFixed(1) + "s";
      const spawn_random = Math.floor(Math.random() * o.spawnsvg_uncertainty);
      const rotation =
        o.min_rotation +
        Math.floor(Math.random() * (o.max_rotation - o.min_rotation));
      console.log(
        `new svg; duration_css is ${duration}; x is ${x}; spawntime is ${spawn_random}, rotation is ${rotation}degs`
      );
      ...
      svg.setAttribute("style", css_string);
      setTimeout(
        () => svg.parentNode.removeChild(svg),
        spawn_random + duration + 500
      );
      return { svg: svg, spawn_random: spawn_random, rotation: rotation };
    };
    setInterval(() => {
      const svg = create_svg();
      subcontainer.appendChild(svg.svg);
      const minus_or_not = !Math.round(Math.random()) ? "-" : "";
      setTimeout(() => {
        svg.svg.style.transform = `translate(0px, ${h +
          120}px) rotate(${minus_or_not}${svg.rotation}deg)`;
        svg.svg.style.backgroundColor = o.end_color;
      }, 100 + svg.spawn_random);
    }, 1e4 / o.density);
  }
